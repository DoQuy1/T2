...
<%
    ...
    Else
        ...
        statuspd = Request.form("statuspd")

        if (isnull (idproduct) OR trim(idproduct) = "") then idproduct=0 end if

        if (cint(idproduct)=0) then
            if (NOT isnull(name) and name<>"" and NOT isnull(description) and description<>"" and NOT isnull(price) and price<>"" and NOT isnull(brand) and brand<>"" and NOT isnull(category) and category<>""  and NOT isnull(statuspd) and statuspd<>"") then
                Set cmdPrep = Server.CreateObject("ADODB.Command")
                cmdPrep.ActiveConnection = connDB
                cmdPrep.CommandType = 1
            ...
            if (NOT isnull(name) and name<>"" and NOT isnull(description) and description<>"" and NOT isnull(price) and price<>"" and NOT isnull(brand) and brand<>"" and NOT isnull(category) and category<>"" and NOT isnull(statuspd) and statuspd<>"") then
                ...
                cmdPrep.CommandType = 1
                cmdPrep.Prepared = True
                cmdPrep.CommandText = "UPDATE Products SET CategoryID=?,ProductName=?,Description=?,Price=?,Image=?,Brand=?,Status=? WHERE ProductID=?"
...
<!-- #include file="./layout/header.asp" -->
        ...
        <div class="container mb-5">
            <form method="post">
                ...
                <div class="mb-3">
                    <label for="statuspd" class="form-label">Status</label>
                    <input type="text" class="form-control" id="status" name="statuspd"value="<%=statuspd%>">
                </div> 
