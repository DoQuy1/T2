...
' trang hien tai
    ...
    ' Lấy giá trị mới của pageSize từ form
        ...
        If Not IsEmpty(newPageSize) Then
            ...
            Session("pageSizeStatistics") = newPageSize
        End If
    End If
    Dim pageSize
    pageSize = Session("pageSizeStatistics")
    ...
    if(trim(fromDate) <> "") and (NOT IsEmpty(fromDate)) and trim(toDate) <> "" and (NOT IsEmpty(toDate)) then
        ...
        currentUrl = "statistics.asp?from_date="&fromDate&"&to_date="&toDate&"&"
    end if
    ...
    End Select
        currentUrl = "statistics.asp?input-search="&inputsearch&"&option-search="&optionsearch&"&"
    end if
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-sm-6">
                           <form class="form-inline" action="" style="justify-content: flex-end;margin-right: 35px;">
                    ...
                    <div class="row">
                        ...
                        if(trim(inputsearch) = "") and (IsEmpty(inputsearch)) and (trim(optionsearch) = "") and (IsEmpty(optionsearch)) and (trim(toDate) = "") and (IsEmpty(toDate)) and (trim(fromDate) = "") and (IsEmpty(fromDate)) then
                            ...
                            Set temp = connDB.execute(str)
                            totaltemp = 0
                            do while not temp.EOF
                                totaltemp = totaltemp + CDbl(temp("TotalAmount"))
                            temp.MoveNext
                            loop
                        %>
                            <h1>Total Revenue:<%=totaltemp%></h1>
                        <%
                        end if
                        if(trim(inputsearch) <> "") and (NOT IsEmpty(inputsearch)) and trim(optionsearch) <> "" and (NOT IsEmpty(optionsearch)) then
                            Select Case optionsearch
                            ...
                            str = "SELECT * FROM Orders "
                            Case 1 
                            str = "SELECT * FROM Orders Where CustomerID IN(Select CustomerID From Customers where Name Like '%"&inputsearch&"%') "
                            Case 2
                            str = "SELECT * FROM Orders Where CustomerID In(Select CustomerID From Customers where Username Like '%"&inputsearch&"%') "
                            Case 3
                            str = "SELECT * FROM Orders Where PaymentMethodID In (Select PaymentID From PaymentMethods where PaymentMethodName Like '%"&inputsearch&"%') "
                            Case 4
                            str = "SELECT * FROM Orders Where Status '%"&inputsearch&"%' "
                            Case 5
                            str = "SELECT * FROM Orders Where ShippAddress '%"&inputsearch&"%' "
                            Case 6
                            str = "SELECT * FROM Orders Where Price <= "&inputsearch&" "
                            End Select
                            Set temp = connDB.execute(str)
                            totaltemp = 0
                            do while not temp.EOF
                        ...
                        if(trim(fromDate) <> "") and (NOT IsEmpty(fromDate)) and trim(toDate) <> "" and (NOT IsEmpty(toDate))then
                         str = "SELECT * FROM Orders Where OrderDate >='"&fromDate&"' AND OrderDate <= '"&toDate&"' "
                            Set temp = connDB.execute(str)
                            totaltemp = 0
                            do while not temp.EOF
                                totaltemp = totaltemp + CDbl(temp("TotalAmount"))
        ...
        <div class="container mt-4">
                ...
                %>
                    <div class="col-ms-12 col-md-5 " >Showing product <%=offset+1%> to <%=pageSize%> of <%=totalRows%></div>
                <%
                    Else
                %>
